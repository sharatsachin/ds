...
2. [Query Understanding & Query Rewriting](#query-understanding--query-rewriting)
3. [Chunking Strategies](#chunking-strategies)
4. [Embedding Models](#embedding-models)
5. [Vector Databases & Indexing](#vector-databases--indexing)
6. [Metadata Filtering & Access-Control-Aware Retrieval](#metadata-filtering--access-control-aware-retrieval)
7. [Similarity Metrics](#similarity-metrics)
8. [Retrieval Evaluation Metrics](#retrieval-evaluation-metrics)
9. [Hybrid Search & Reranking](#hybrid-search--reranking)
10. [Multi-Hop Reasoning & Multi-Document Retrieval](#multi-hop-reasoning--multi-document-retrieval)
11. [Context Compression](#context-compression)
12. [RAG Failure Modes](#rag-failure-modes)
13. [LangChain Core Concepts](#langchain-core-concepts)
14. [LangGraph: State Graphs & Cyclic Workflows](#langgraph-state-graphs--cyclic-workflows)
15. [Tool Calling / Function Calling](#tool-calling--function-calling)
16. [Agent Memory](#agent-memory)
17. [Multi-Agent Orchestration Patterns](#multi-agent-orchestration-patterns)
18. [Guardrails for Agentic Systems](#guardrails-for-agentic-systems)
19. [Hallucination Detection & Mitigation](#hallucination-detection--mitigation)
20. [LLM-as-a-Judge vs Human Evaluation](#llm-as-a-judge-vs-human-evaluation)
21. [General Benchmark Suites](#general-benchmark-suites)
22. [Task-Specific Evaluation Metrics](#task-specific-evaluation-metrics)
23. [A/B Testing GenAI Features](#ab-testing-genai-features)
24. [Latency/Cost Tradeoffs in Deployment](#latencycost-tradeoffs-in-deployment)
25. [Caching Strategies](#caching-strategies)
26. [Streaming vs Batch Generation](#streaming-vs-batch-generation)
27. [Rate Limiting, Retries & Fallbacks](#rate-limiting-retries--fallbacks)
28. [Monitoring LLM Systems in Production](#monitoring-llm-systems-in-production)
29. [Responsible AI in LLM Pipelines](#responsible-ai-in-llm-pipelines)
30. [Connecting to MLOps Practices](#connecting-to-mlops-practices)
31. [Popular GenAI Interview Questions — Full Answers](#popular-genai-interview-questions--full-answers)
32. [Quick Recall Sheet](#quick-recall-sheet)
...
The raw user query is frequently a bad retrieval query. Users phrase requests conversationally and elliptically ("what about the other plan?", "does that apply to me?"), often referencing earlier conversation turns, while retrieval is a similarity match against how source *documents* are actually written — usually more formal, declarative, answer-shaped prose. This mismatch is sometimes called the **query-document semantic gap**: "why is my invoice wrong" and "Billing discrepancies typically arise from proration adjustments applied mid-cycle" can express related content while landing meaningfully far apart in embedding space, because a question and its answer are different *kinds* of text even when they're topically about the same thing. Several techniques transform or multiply the query before it ever reaches the index, specifically to close this gap.

**Query rewriting / reformulation:** pass the raw query (plus recent conversation history, for multi-turn systems) through an LLM (or a smaller/cheaper fine-tuned rewriter) that resolves coreference ("cancel that" → "cancel the Premium subscription"), expands abbreviations and fixes typos, and restates the request in clearer, more retrieval-friendly language before it's embedded or used for BM25 matching. This is the same coreference-resolution problem discussed under [Agent Memory](#agent-memory), applied specifically to the retrieval step rather than to conversational continuity.

**Query expansion:** broaden the query with related terms — synonyms, domain-specific alternate phrasings, or LLM-generated related keywords — and retrieve using the expanded query (or the union of results from the original and expanded forms). This matters most for sparse/BM25 retrieval, which only matches literal tokens: a query for "cancel subscription" won't match a document that only uses the word "termination" unless expansion bridges the two.

**HyDE (Hypothetical Document Embeddings):** instead of embedding the user's raw *question* and searching for documents near it, prompt an LLM to first generate a **hypothetical answer** to the question — a plausible-sounding passage that *would* answer it, even though the LLM is generating it purely from parametric knowledge with no guarantee it's factually correct — and embed *that* hypothetical answer, then search the vector index using the hypothetical-answer embedding instead of (or blended with) the raw-question embedding. This works because it reframes the retrieval query from question-style text into answer-style text, which is exactly the register that real indexed documents are written in — so the hypothetical answer's embedding lands much closer to genuinely matching documents than the bare question's embedding would, even though the hypothetical answer itself may contain fabricated specifics. Crucially, the hallucinated content in the hypothetical document is never shown to the user or fed into final generation — it exists purely as a throwaway retrieval proxy; the actual generation step still conditions only on the real retrieved chunks. HyDE tends to help most on open-ended "why"/"how" queries where the question and answer are stylistically very different, and helps less (or can even hurt) on queries that are already closely aligned with document phrasing, such as exact keyword/entity/code lookups — which is also why it's usually paired with hybrid search rather than used as a full replacement for it.

**Multi-query retrieval:** rather than committing to one (possibly imperfect) reformulation, prompt the LLM to generate several *different* reformulations of the original query — rephrasing it from different angles, or generating the sub-questions a complex query implies — retrieve independently for each reformulated query, then merge and deduplicate the combined result set (e.g., via reciprocal rank fusion, the same fusion mechanism used in [hybrid search](#hybrid-search--reranking)) before reranking. This hedges against any single reformulation missing the mark, at the cost of running retrieval N times instead of once per query.

**Query routing:** in systems backed by multiple indices, retrievers, or tools (a product-docs index, a legal-policy index, a live database, a web-search tool), a routing step — typically a lightweight classifier or an LLM prompted to categorize the query — decides *which* retriever(s)/tool(s) a given query should go to, rather than fanning every query out to every source indiscriminately. This keeps retrieval both cheaper (skip irrelevant indices) and more precise (avoid diluting results with irrelevant-source noise), and is a prerequisite for any multi-source RAG system to scale past a single knowledge base — it's conceptually the same "classify then dispatch" pattern used in the supervisor-worker [multi-agent pattern](#multi-agent-orchestration-patterns), applied to retrieval sources instead of agent workers.

```mermaid
flowchart LR
    Q[Raw User Query] --> RW[Rewrite / Resolve Coreference]
    RW --> RT{Query Router}
    RT -->|docs index| HyDE[HyDE: generate hypothetical answer, embed it]
    RT -->|policy index| MQ[Multi-Query: generate N reformulations]
    HyDE --> R1[Retrieve]
    MQ --> R2[Retrieve per reformulation]
    R2 --> MG[Merge + Dedupe]
    R1 --> RR[Rerank]
    MG --> RR
    RR --> LLM[LLM Context]
...
**Interview angle:**
- *"What is HyDE and why would embedding a hallucinated answer improve retrieval?"* — Model answer: HyDE prompts the LLM to write a hypothetical answer to the user's question, then embeds that hypothetical answer instead of the raw question and uses it as the retrieval query. It works because embedding models place text close together based on semantic *and stylistic* similarity, and a question is a different register of text than the answer-style prose that actual documents are usually written in — so a raw question can embed surprisingly far from the document that answers it. The hypothetical answer, even though it may contain fabricated specifics, is written in the same answer-like register as real documents, so its embedding tends to land much closer to genuinely relevant chunks. The key safety property is that the hallucinated text is only ever used as an internal, throwaway retrieval query — it's never shown to the user and never enters the final generation context, so its factual inaccuracy doesn't propagate into the answer, only its *style* does the useful work.
- *"When would you use multi-query retrieval versus a single rewritten query?"* — Model answer: A single rewrite is cheaper and fine when the query is fairly unambiguous — mostly fixing coreference or phrasing. Multi-query retrieval earns its extra cost when a query is genuinely ambiguous or multi-faceted, since generating several different reformulations and retrieving for each hedges against any one reformulation missing the right documents, and merging (typically via RRF, the same fusion used in hybrid search) tends to raise recall meaningfully at the cost of running the retrieval step N times. I'd reserve it for queries flagged as complex/ambiguous by a routing step rather than applying it universally, since the extra retrieval calls add both latency and cost.
...
Plain chunking forces a single tradeoff onto one chunk size: small chunks embed precisely (a short, focused chunk produces a sharp embedding that matches a specific query well) but often lack enough surrounding context for the LLM to generate a complete answer from; large chunks give the LLM plenty of context but produce a blurrier, less discriminative embedding, since the vector is now an average over multiple ideas. **Parent-child (small-to-big) retrieval** decouples the two roles instead of picking one chunk size for both: index small, precise **child** chunks (e.g., individual sentences or small ~100-200 token windows) for the embedding-similarity search itself, but link each child chunk back to a larger **parent** chunk or whole section it belongs to; when a child chunk is retrieved as a similarity match, the pipeline fetches and passes the *parent* — not the child — into the LLM's context.

| | Retrieve small, generate on small | Retrieve small, generate on parent (small-to-big) | Retrieve large, generate on large |
|---|---|---|---|
| Embedding precision | High — sharp, focused match | High — same precise child-level match | Lower — blurred, multi-topic embedding |
| Context sufficiency for generation | Often insufficient — LLM only sees the narrow matched sentence | High — LLM sees the full surrounding section | High, but only by sacrificing retrieval precision |
| Context-window cost | Low | Moderate — only the top-matching few parents are expanded | High — large chunks retrieved directly |
| Typical implementation | Naive single-level chunking | Explicit parent-child index (e.g., LangChain's `ParentDocumentRetriever`, or a `parent_id` link in chunk metadata) | Naive single-level chunking with a large chunk size |

Mechanically: at ingestion, split each document into large parent chunks (e.g., full sections, ~2000 tokens), then further split each parent into small child chunks (single sentences or ~100-200 token windows) and embed *only* the children, storing each child's embedding alongside a pointer to its parent's ID/content. At query time, similarity search runs over child embeddings as usual (getting the precision benefit), but the retrieval step resolves each matched child back to its parent, deduplicates parents (multiple matched children can belong to the same parent), and passes the resolved parent chunks into the LLM's context — giving the model the full surrounding passage needed to actually answer the question, not just the narrow sentence that happened to match best. It's called "small-to-big" precisely because the unit that's *searched* is small and the unit that's *generated from* is big.

```mermaid
flowchart LR
    Doc[Document] --> P1[Parent Chunk 1<br/>~2000 tokens]
    Doc --> P2[Parent Chunk 2<br/>~2000 tokens]
    P1 --> C1[Child: sentence A]
    P1 --> C2[Child: sentence B]
    P2 --> C3[Child: sentence C]
    Q[Query] -.embed & search.-> C2
    C2 -.resolve to parent.-> P1
    P1 --> LLM[LLM Context]
```
...
- *"How would parent-child retrieval help a RAG system that keeps retrieving the 'right neighborhood' but generating incomplete answers?"* — Model answer: That symptom — the matched chunk is topically on-target but the answer is thin or missing surrounding detail — is the classic small-chunk-context tradeoff, and parent-child retrieval fixes it directly: keep indexing and searching small, precise child chunks (so retrieval precision stays high) but resolve each match back to its larger parent section before it goes into the LLM's prompt, so generation has full context even though the match itself was narrow. I'd implement this as an explicit parent-child link at ingestion time (each child chunk stores a pointer to its parent's ID) rather than trying to solve it by simply making chunks bigger, since bigger chunks would trade away the retrieval precision that was working fine in the first place.
...
## Metadata Filtering & Access-Control-Aware Retrieval

Vector search rarely runs in isolation — production retrieval almost always needs to be scoped by **metadata**: only search documents of a certain type, from a certain source, within a date range, or belonging to a specific tenant/customer. Most vector databases support attaching arbitrary metadata (tags, timestamps, source, document type, tenant ID, permission labels) to each vector at index time and filtering on it at query time.

**Pre-filtering vs post-filtering:**

| | Pre-filtering | Post-filtering |
|---|---|---|
| When the filter applies | Before or during the ANN search itself — the index only considers candidates matching the filter | After the ANN search returns its top-k — non-matching results are discarded from an already-fixed candidate set |
| Recall risk | Low — the search explores the full space of filter-matching candidates | High — if most of the true top-k nearest neighbors don't match the filter, discarding them after the fact can leave you with fewer (or zero) usable results even though better matches existed further down the ranked list |
| Performance | Depends on the index's native filter support — a well-implemented filtered ANN search (e.g., filtering integrated into HNSW graph traversal) stays fast | Simple to implement (just discard non-matching hits), but wastes the ANN search's effort on candidates that get thrown away, and needs a larger initial `k` fetched to compensate |
| Support | Native, first-class in most managed vector DBs (Pinecone, Weaviate) | A common bolt-on when a filter is layered on top of a library with no native filtering (e.g., raw FAISS) |

The under-return risk is the key interview point: if you ask for the top 10 semantically similar chunks *and then* filter down to only those tagged `department: finance`, and it turns out only 2 of the unfiltered top 10 happen to be finance documents, post-filtering leaves you with just 2 results even though plenty of relevant finance documents ranked 11th-50th may exist — the ANN search had no idea it needed to prioritize finance documents at all. Pre-filtering (constraining the search itself to the finance-tagged subset before or during the nearest-neighbor search) avoids this entirely.

**Authorization-aware retrieval (the enterprise security angle):** in an enterprise RAG system, different requesting users are authorized to see different subsets of the underlying documents — the same knowledge base might mix public docs, department-restricted docs, and documents scoped to specific customer accounts. If retrieval is not aware of who's asking, it will happily surface (and let the LLM summarize/quote from) a document the requesting user has no right to see, purely because it's semantically similar to their query — this turns RAG into an accidental **privilege-escalation path**, since a similarity search has no inherent concept of permissions, and an LLM cannot reliably be trusted to notice and refuse to use a document it was simply *given* in context. Never rely on prompting the model to "not use this if the user isn't authorized" — by the time an unauthorized chunk is in the prompt, the leak has already happened at the retrieval layer, and the model may repeat its contents regardless of instructions.

The correct pattern makes the requesting user's actual entitlements part of the retrieval query itself, enforced by a layer *outside* the LLM/retriever:

- **Filter at the vector-DB query level, using the requester's entitlements as a metadata filter** — resolve the requesting user's permissions (document ACLs, role, department, customer/tenant scope) *before* the retrieval call, translate them into a metadata filter (e.g., `tenant_id == X AND acl_group IN [user's groups]`), and apply it as a pre-filter on the ANN search itself, so unauthorized documents are never even candidates — not retrieved-then-hidden, simply never surfaced.
- **Filtering after retrieval is weaker and leakier** — checking permissions only after the top-k comes back means the ranked list, the shortlist, and any logs/traces of the retrieval step may already contain unauthorized content; it also suffers the same recall problem as post-filtering in general (the true authorized top-k can get crowded out by unauthorized-but-more-similar documents that occupied the slots before the permission check ran).
- **Per-tenant index isolation vs. shared index with tenant-ID metadata filtering** — a fully separate vector index per tenant/customer gives the strongest isolation guarantee (there's no shared search space to leak across at all, so a filter bug can't cross a tenant boundary because there's no path to the wrong tenant's data) at the cost of more operational overhead (many small indices to provision, scale, and keep in sync) and worse resource utilization; a shared index with a mandatory tenant-ID (and finer-grained ACL) filter on every query is more efficient to operate but pushes correctness onto "the filter is always applied, on every code path, with no exceptions" — a single missed filter application anywhere in the codebase becomes a cross-tenant data leak.
- **Authorization must be enforced outside the LLM/retriever, never delegated to the model** — the same principle as least-privilege access control in traditional systems: permission checks belong in the retrieval/data-access layer (the same layer that would enforce row-level security in a traditional database), as a hard filter applied before anything reaches the model, not as a prompt instruction hoping the model self-censors. This mirrors the general agent-authorization principle (external entitlement checks around anything an agent can access or do, never trust placed in the model's own judgment) covered in the security-fundamentals file — here it's applied specifically to the retrieval step of a RAG pipeline rather than to agent tool-calling actions.

**Interview angle:**
- *"How would you prevent a RAG chatbot from leaking a document a user isn't authorized to see, given the document is semantically relevant to their question?"* — Model answer: The fix has to happen at retrieval time, not at the prompt/generation level — I'd never rely on instructing the model "don't use this document if unauthorized," because by the time an unauthorized chunk is sitting in context, the leak has already occurred and the model may repeat it regardless of instructions. Concretely, I'd resolve the requesting user's entitlements (role, department, account/tenant scope, document-level ACLs) before the retrieval call, translate that into a metadata filter, and apply it as a *pre-filter* on the vector search itself — so unauthorized documents are never candidates for the ANN search at all, rather than being retrieved and then screened out afterward, which is both leakier (they briefly exist in the retrieved set/logs) and worse for recall (they can crowd out authorized documents from the top-k before any check runs). For multi-tenant systems specifically, I'd evaluate per-tenant index isolation for the highest-sensitivity data, and a shared index with a mandatory, defense-in-depth-tested tenant/ACL filter for everything else — treating "the entitlement filter is applied on every single retrieval code path with zero exceptions" as a security invariant to test explicitly, the same way you'd test row-level security in a traditional multi-tenant database.
...
Even after hybrid retrieval and reranking narrow things down to a shortlist, feeding all of that shortlist's raw text into the LLM's context isn't free — every retrieved token costs money and latency (both scale with input tokens), and, more subtly, stuffing a long, retrieved-heavy context in front of the model can trigger **"lost in the middle"** degradation, where information positioned in the middle of a long context is attended to and recalled less reliably than information at the very start or end, even when it's the actually-relevant chunk. **Context compression** is the practice of reducing retrieved context to just what's needed *before* it reaches the generation call, rather than passing every retrieved chunk through in full.

**Extractive compression:** from each retrieved chunk, select only the sentences/spans that are actually relevant to the query (typically via a smaller scoring model, or the same embedding similarity used in retrieval but applied at sentence granularity within the chunk), and discard the rest of the chunk's text. This keeps only original source text (nothing is paraphrased, which helps faithfulness/citation accuracy) at the cost of only ever removing whole spans — it can't compress a genuinely-relevant sentence any further.

**Abstractive compression (summarization):** run retrieved chunks through a smaller/cheaper LLM whose only job is to summarize or compress them down relative to the query (e.g., "summarize the parts of this passage relevant to: {query}"), producing a shorter synthesized passage to feed into the main, more expensive generation call. This can compress more aggressively than extractive methods (it can condense several redundant sentences into one), but introduces an extra LLM call (added latency/cost of its own, though typically on a cheaper model) and a risk that the compression step itself drops or distorts a relevant detail before the main model ever sees it — so compression quality has to be evaluated, not assumed.

**Contextual compression retrievers:** an architectural pattern (popularized by LangChain's `ContextualCompressionRetriever`) that wraps a base retriever with a compressor step: the base retriever runs its normal retrieval (dense/hybrid + optional reranking) to get candidate chunks, and a compressor component then processes each returned chunk before it's handed to the generation step — the compressor can be an extractive filter, an LLM-based summarizer, or a redundancy filter that drops chunks that are near-duplicates of ones already kept. The retriever-plus-compressor is composed as a single retrieval interface from the caller's point of view, so compression becomes a transparent post-processing stage on top of whatever retrieval pipeline was already there.

**Reranking as implicit compression:** cross-encoder reranking (see [Hybrid Search & Reranking](#hybrid-search--reranking)) is itself a form of compression — cutting a shortlist of 50-100 candidates down to the true top-N before generation reduces the token count going into the LLM exactly the way extractive/abstractive compression does, just at chunk granularity rather than sentence/span granularity. In practice, the two are usually layered: rerank to cut chunk count, then optionally compress within the surviving chunks to cut token count further, since reranking alone doesn't shrink an individual chunk that's long but only partially relevant.

**Interview angle:**
- *"Why would you add a compression step if reranking already shortlists the best chunks?"* — Model answer: Reranking controls *how many* chunks make it into context, but doesn't touch what's inside each surviving chunk — a chunk can be correctly identified as the most relevant one available and still be mostly irrelevant text once you look at it sentence by sentence, especially with larger or loosely-chunked source documents. Compression operates one level down: extractive compression pulls just the relevant sentences out of each surviving chunk, and abstractive compression synthesizes a shorter, denser passage from it. Beyond just saving tokens/cost, I'd point to "lost in the middle" — even chunks that earned their place in context can get under-attended if they're padded with irrelevant sentences pushing the truly relevant span into a less-attended position in a long prompt, so compression is also a quality lever, not purely a cost one. I'd typically layer both: rerank to cut which chunks survive, then compress what's inside them, validating each stage against the retrieval/faithfulness eval harness to make sure compression isn't quietly dropping details the answer needs.
...
| Context window overflow | Too many chunks retrieved, or chunks too long, exceeding the LLM's context limit or crowding out the generation budget | Tighter reranking to shortlist only the best chunks, summarize/compress retrieved context before passing to the LLM (see [Context Compression](#context-compression)), use models with larger context windows judiciously (not as a substitute for good retrieval) |
...
| Noisy retrieval crowding out relevant chunks | Retrieving too broadly (large k) or with too loose a similarity threshold surfaces chunks that are topically adjacent but not actually useful, occupying context-window slots and forcing the LLM to sift signal from noise, which measurably increases the chance it grounds its answer in the wrong passage or produces a diluted, hedged answer | Tighter reranking/cross-encoder shortlisting, lowering retrieved-k to only what's needed, context compression to strip irrelevant spans, evaluating precision@k alongside recall@k so retrieval isn't quietly over-fetching |
| Duplicate/redundant retrieved chunks | Overlapping chunking windows, near-duplicate source documents, or multiple document versions in the corpus cause several retrieved chunks to restate the same fact — wasting context-window budget on repetition instead of coverage, and risking subtly *different* (even contradictory) phrasings of the same fact confusing the model about which version is authoritative | Deduplicate retrieved results (near-duplicate detection via embedding similarity between candidate chunks before they're added to final context), corpus-level dedup/versioning at ingestion, a redundancy-aware compressor (see [Context Compression](#context-compression)) that drops chunks adding no new information beyond what's already selected |
...
**Designing good tool/function schemas:** each tool is exposed to the model as a name, a natural-language description, and a structured parameter schema (typically JSON Schema — field names, types, which fields are required, enums for constrained values, and a description per field). The description text matters far more than it might seem: the model picks which tool to call, and how to fill its arguments, purely by matching the user's request against the tool descriptions it was given — so a vague or overlapping description ("search" vs "lookup" vs "query", all described similarly) increases the odds of the model picking the wrong tool or misfilling a parameter, while a precise, disambiguating description ("search only the internal knowledge base of product documentation; do not use for order-specific or account-specific questions") sharply narrows when the model should reach for it. In practice, schema design is itself a tuning surface: ambiguous tool boundaries, overlapping tool names, or parameter descriptions that don't specify format/units (is `date` an ISO string, a Unix timestamp, `MM/DD/YYYY`?) are common, underrated sources of tool-calling failures that show up as symptoms elsewhere (wrong tool chosen, malformed arguments) but are actually schema-authoring problems, fixable by tightening the schema rather than by patching the orchestration code around it.

**Tool selection accuracy as a real failure point:** three distinct failure modes show up in practice, and it's worth distinguishing them because they call for different fixes:
- **Wrong tool chosen** — the model picks a plausible-but-incorrect tool among several similar ones (e.g., a "get current weather" tool called for a historical-weather question that needed a different tool), usually a symptom of overlapping/ambiguous tool descriptions or too many tools exposed for the model to reliably discriminate between.
- **Hallucinated tool calls** — the model emits a call to a tool name that was never actually provided in its schema list, effectively inventing a plausible-sounding function because the true available tools didn't obviously cover the request. The orchestration layer must always validate the tool name against the actual registered tool set and reject/reprompt on anything that doesn't match — never attempt to "guess" what the model meant and dispatch to a similarly-named real tool instead.
- **Malformed arguments** — the call targets a real, correct tool, but the arguments don't conform to its schema (wrong type, missing required field, an out-of-range/invalid enum value), which is common when the user's request under-specifies information the schema requires and the model fills the gap with a plausible-looking but invalid guess rather than asking a clarifying question.

**Validating tool call arguments before execution:** never execute a tool call directly off the model's raw output — validate the arguments against the tool's declared schema (types, required fields, ranges/enums) and any domain-specific business rules beyond what a JSON Schema alone can express (e.g., "an order ID must reference an order actually belonging to this requesting user") *before* the tool runs. On a validation failure, either reject-and-reprompt the model with the specific validation error (so it can self-correct — this works well for one-off malformed-argument slips) or fail closed for anything security/business-critical, rather than attempting to coerce an invalid call into something executable. This is the same output-validation discipline covered under [Guardrails for Agentic Systems](#guardrails-for-agentic-systems), applied specifically at the tool-call boundary rather than at final-answer time.

**Retry logic on tool failures:** a tool call can fail for reasons that have nothing to do with whether the model chose correctly — a transient network error, a downstream API rate limit, a momentary database timeout — so tool-calling infrastructure needs its own retry policy, distinct from (and layered underneath) the model's own reasoning loop: capped exponential backoff with jitter for transient/retryable errors (mirroring the same pattern from [Rate Limiting, Retries & Fallbacks](#rate-limiting-retries--fallbacks)), while non-retryable errors (a validation failure, a permissions error, a 4xx-style "this request is fundamentally invalid") should be surfaced back to the model as an observation immediately rather than blindly retried, since retrying a call that's wrong for a reason retrying can't fix just wastes latency and quota.

**Timeouts for slow/hanging tools:** every tool call needs an enforced timeout independent of retry logic, because a tool that hangs (rather than erroring cleanly) can otherwise stall the entire agent loop indefinitely — a slow downstream API, a long-running query, or a network partition should fail the call after a bounded wait rather than leaving the agent (and the user-facing request behind it) hanging. The timeout value should typically be tool-specific (a fast internal lookup and a slow external enrichment API don't deserve the same budget), and its expiry should feed back into the same retry/fallback logic as any other tool failure, not be treated as a special, unhandled case.

**Idempotency of tool calls — designing tools to be safely retryable:** retry logic is only safe to apply if the tool itself is **idempotent** — calling it twice with the same arguments produces the same end state as calling it once, rather than compounding the side effect. This matters acutely for any tool with a real-world side effect: a "send refund," "charge card," or "send email" tool that isn't idempotent can double-charge, double-refund, or double-send if a retry fires after the first call actually succeeded but its success response was lost to a network error before the orchestration layer ever saw it — a very common failure shape, where the action happened but the caller couldn't tell. The standard fix is to design the tool's API (not just the calling code) around an **idempotency key**: the caller generates a unique key per logical action attempt and passes it with the call, and the tool's backend deduplicates on that key — a repeated call with the same key is recognized as a retry of the same logical action and returns the original result without re-executing the side effect, rather than performing it again. Read-only tools (search, lookup, calculation) are naturally idempotent and need no special handling; any tool that writes, charges, sends, or mutates state should be treated as unsafe to retry blindly unless it explicitly supports idempotency keys or an equivalent dedup mechanism — retry logic for such tools should default to *not* retrying automatically (surfacing the ambiguous-outcome failure for explicit resolution, e.g. "check whether the refund actually went through before retrying") rather than assuming a retry is harmless.
...
- *"Your agent occasionally calls a tool that doesn't exist in its schema, or passes malformed arguments. How do you prevent that from causing real harm?"* — Model answer: The orchestration layer, not the model, is the enforcement point — I always validate the tool name against the actual registered tool set before doing anything with a call (a call to an unregistered/hallucinated tool name gets rejected and reprompted, never executed against something the caller guesses at), and I validate arguments against the tool's declared schema plus any domain business rules before execution, rejecting or reprompting on failure rather than trying to coerce an invalid call into a runnable one. For anything with a real side effect (money movement, sends, writes), I'd also require the tool to be idempotent via an idempotency key, specifically so that any retry — whether triggered by my own retry logic after a transient error, or by the model deciding to "try again" — can never double-execute the action even in the worst case where a first call actually succeeded but its response was lost.
- *"Why does idempotency matter specifically for tool calls made by an LLM agent, more than for a typical backend API?"* — Model answer: Because an LLM-driven agent has more paths that can trigger an unintended retry than a typical deterministic backend does — my own infrastructure-level retry logic on transient errors is one, but the model itself can also decide, based on an ambiguous or slow tool response, to "try that again," and a confused reasoning step can lead to a duplicate call that a human engineer calling the same API directly simply wouldn't produce. Since I can't fully guarantee the model will never re-issue a logically-duplicate call, any tool with a real side effect needs to be safe to call twice with the same arguments — enforced via an idempotency key at the tool's own API layer — rather than relying on the orchestration layer or the model to perfectly prevent duplicate calls from ever happening in the first place.
...
- **Query understanding:** raw queries often embed far from the answer-style documents that match them (semantic gap); query rewriting fixes coreference/phrasing; HyDE embeds an LLM-generated *hypothetical answer* (never shown to the user) instead of the raw question, since answer-style text matches document-style text better; multi-query retrieval generates several reformulations and fuses (RRF) their results; query routing classifies a query to pick the right index/tool in multi-source RAG.
...
- **Parent-child (small-to-big) retrieval:** embed and search small precise child chunks (sentences/small windows) for retrieval accuracy, but resolve matches to and generate on the larger parent chunk/section for context sufficiency — decouples retrieval precision from generation context size.
...
- **Metadata filtering:** pre-filter (constrain the ANN search itself) beats post-filter (discard after the fact — can under-return results when top-k gets crowded out by non-matching hits before the filter runs).
- **Access-control-aware retrieval:** retrieval must enforce the requesting user's actual entitlements (document ACLs/row-level security) as a metadata pre-filter at the vector-DB query level — never trust the LLM to self-censor an unauthorized document once it's in context; per-tenant index isolation = strongest isolation, shared index + tenant-ID filter = more efficient but the filter must be applied on every code path with zero exceptions.
...
- **Context compression:** shrink retrieved context before generation to cut cost/latency and avoid "lost in the middle" — extractive (keep only relevant sentences/spans), abstractive (summarize chunks via a cheaper LLM), contextual compression retrievers (retriever + compressor pattern), reranking itself is implicit compression.
- **RAG failure modes:** irrelevant retrieval, context overflow (fix: better reranking/compression), stale/version-mismatched embeddings (fix: re-index on change, pin/migrate embedding versions), noisy retrieval crowding out good chunks (fix: tighter k/reranking, precision@k), duplicate/redundant chunks wasting context or conflicting (fix: dedupe retrieved results, corpus-level versioning).
...
- **Tool calling loop:** model given tool schemas → decides call needed → structured call → tool executes → result fed back as observation → model continues/answers. Schema descriptions drive selection accuracy; validate tool name + arguments (schema + business rules) before execution — never trust a call at face value (guards against wrong tool, hallucinated tool, malformed args); retry only idempotent/transient failures with backoff, enforce per-tool timeouts, and require an idempotency key on any tool with a real side effect (charge, refund, send, write) so a retry can never double-execute.
