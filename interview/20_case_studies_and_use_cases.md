...
This file is a companion to `13_system_design_ml.md`, not a replacement for it. File 13 gives you **6 full deep-dive system designs**, each worked through an 8-step framework in real depth — read it when you want to practice going deep on a small number of flagship designs.

This file does the opposite: it's a **wide, shallow bank of ~30 shorter case-study prompts** — the "case interview meets product sense meets ML" style questions that show up in rapid-fire rounds, product-sense screens, and "how would you approach this" segments of a broader interview. Where two prompts sound similar to a design in file 13 (duplicate detection, churn, search/ranking), this file deliberately takes a **different angle or a different sub-question** so the two files complement rather than repeat each other.

**How to use this file:** don't read straight through. For each prompt, cover the answer below with your hand, spend **2-3 minutes talking your own structured answer out loud** — objective, clarifying questions, data, approach, risks, validation — and only then read the given approach and compare. The value here is volume and pattern recognition across scenarios, not maximal depth on any one of them; if you want to go deep on a specific scenario afterward, that's what file 13 (and the topic-specific files elsewhere in this kit) are for.
...
- [G. Agentic AI / Security Platform Case Studies](#g-agentic-ai--security-platform-case-studies)
  - [G1. Reducing Alert Fatigue in a SOC with a Multi-Agent System](#g1-reducing-alert-fatigue-in-a-soc-with-a-multi-agent-system)
  - [G2. Authorization Boundary for an Autonomous Remediation Agent](#g2-authorization-boundary-for-an-autonomous-remediation-agent)
  - [G3. Access-Control Leak in a RAG-Powered Security Co-Pilot](#g3-access-control-leak-in-a-rag-powered-security-co-pilot)
...
**Approach:** *(File 13's Additional Questions section already covers the full architecture for marketplace-listing/image duplicate detection — ANN index, embedding fusion, clustering. This answer takes the text/plagiarism angle and leads with the business-decision layer instead of the retrieval architecture.)*
...
**Approach:** *(Distinct from file 13's recommendation-system design — search is query-driven and intent-explicit, versus recommendations being largely query-less and preference-driven; the funnel shape is similar but the signal and cold-start problem differ.)*
...
- **Data & features:** The customer's message, relevant account/order context (retrieved, not hallucinated), and a knowledge base of policy/response templates retrieved via RAG (same retrieval pattern as file 13's support chatbot design, but generating a draft for a human rather than a direct customer-facing answer).
...
**Approach:**
- **Objective/metric:** Total cost of ownership and risk-adjusted quality at the feature's actual production volume — this is a build-vs-buy tradeoff (same underlying logic as file 13's build-vs-vendor question), specialized to LLMs.
...
**Approach:** *(Distinct angle from file 13's demand-forecasting design: that design is about producing the demand *forecast* itself at scale; this one is the downstream inventory-policy decision that consumes a forecast — reorder point and safety stock, not point prediction.)*
...
## G. Agentic AI / Security Platform Case Studies

*(These draw on the agentic-architecture, security-fundamentals, and enterprise-platform-design material in files 14-16 — the framing/judgment layer on top of that deeper reference content, in this file's usual case-study shape.)*

### G1. Reducing Alert Fatigue in a SOC with a Multi-Agent System

**"Your SOC is drowning in alerts and analysts are missing real incidents in the noise — how would you use a multi-agent system to triage this?"**

**Approach:**
- **Objective/metric:** Reduce the volume of alerts that actually reach an analyst's queue without increasing the rate of missed real incidents (false negatives) — the objective is bounded noise reduction, not raw alert-count reduction, since the two are trivially confusable and only one of them is the actual goal.
- **Clarifying questions:** What fraction of current alerts are known duplicates/false positives, and does that vary by rule/source? What enrichment context already exists (identity history, asset criticality, threat-intel feeds)? Is there an existing playbook the team already trusts, or is triage purely ad hoc analyst judgment today?
- **Data & features:** Raw alert metadata, related/correlated alerts likely sharing one root cause, identity and asset context, and historical analyst disposition labels (confirmed-true-positive vs. confirmed-false-positive) as ground truth to calibrate against.
- **Approach (baseline → refinement):** Baseline: a **Blue-style triage agent** that enriches each incoming alert (pulling identity/asset/threat-intel context the way an analyst manually would), deduplicates alerts likely sharing one root cause, and scores severity/confidence against known playbook patterns — auto-closing high-confidence benign alerts and auto-escalating high-confidence malicious ones with a synthesized investigation summary, leaving only the genuinely ambiguous middle band for a human (the same fast-path/slow-path funnel as the content-moderation case study, applied to security triage). Refine by decomposing the triage agent into narrower specialist sub-agents (an identity-signal agent, a network-signal agent) invoked as workers so each enrichment step is auditable and independently improvable, rather than one monolithic agent reasoning over every signal type at once.
- **Risks/tradeoffs:** Over-suppression is the dominant risk — an agent tuned to minimize escalations can quietly auto-close real incidents, and because true incidents are a tiny fraction of total alert volume (the base-rate problem), a modest suppression error can hide a meaningful number of missed threats without moving the aggregate alert-fatigue metric at all; "fewer alerts reaching analysts" is gameable and must never be optimized in isolation from missed-threat rate.
- **Validate impact:** Use the security-agent evaluation layer, not just alert-volume reduction: track missed-threat rate (false negatives) as a hard ceiling via periodic human audit of a sample of auto-closed alerts, track MTTD/MTTR and escalation rate segmented by category (a rising aggregate escalation rate can mean either "harder cases correctly recognized" or "an upstream regression," which only segmentation disambiguates), and apply the "good yesterday, bad today" observability lens — if the triage agent's suppression pattern shifts day-over-day with no corresponding rule/prompt/model change, suspect input-distribution drift (a new attack campaign) rather than assuming the agent quietly got better or worse on its own.

### G2. Authorization Boundary for an Autonomous Remediation Agent

**"Design how you'd let an AI agent take automated remediation actions — like disabling a compromised account or isolating a host — without it becoming a liability."**

**Approach:**
- **Objective/metric:** Minimize dwell time (mean-time-to-remediate) while holding unsafe-action rate — the fraction of automated actions later found to have caused unintended harm — near zero; the objective is bounded autonomy earned over time, not maximum automation from day one.
- **Clarifying questions:** Which specific actions are in scope, and how reversible is each (temporarily isolating a host vs. permanently deleting a resource)? Are there asset classes (regulated data stores, executive/privileged accounts) that should never auto-remediate regardless of confidence? What does the current human-only process look like, as the baseline to beat?
- **Data & features:** Nothing here is a modeling feature in the usual sense — what actually matters is the ground-truth permission/policy record, the investigating agent's confidence/evidence signal, and the historical track record (in shadow mode, then limited live use) of each specific remediation playbook.
- **Approach (baseline → refinement):** The core architectural principle: **the model is never the final authority on whether an action executes** — it can only *propose* a structured action (tool, parameters, target); a separate, deterministic authorization/policy-enforcement service, entirely outside the LLM's own reasoning, independently validates every proposal against real permissions before an actuator carries it out. Baseline: every proposed action requires human sign-off. Refine into a tiered gate by reversibility and blast radius — fully reversible, narrowly-scoped actions (isolate one already-flagged host, with an automatic un-isolate on the next clean check-in) earn autonomous execution only after proving a low unsafe-action rate in shadow mode; irreversible or wide-blast-radius actions (org-wide policy changes, permanent deletion) always require explicit human approval, regardless of model confidence. Scope the agent's own credentials to least privilege (a tool that can only "isolate this host," never generic command execution, using short-lived per-task credentials) specifically to bound the confused-deputy risk — an agent tricked via injected content into misusing its *own legitimate* privilege is only as dangerous as that privilege is broad.
- **Risks/tradeoffs:** Excessive permissions multiply every other failure mode (a poisoned tool description or an injected instruction becomes catastrophic only if the agent's standing privilege is broad); over-restrictive gating just recreates the bottleneck the agent was meant to relieve — the resolution is to widen autonomy only for the specific, narrow scenario types that have actually earned it, never as a blanket upgrade.
- **Validate impact:** Every proposal, policy decision, and outcome logged for audit; every autonomous action paired with a rollback/compensating action so a wrong remediation is a bounded, reversible incident, not a permanent one; unsafe-action rate and human-approval-rate trends reviewed on a recurring cadence, with unsafe-action rate treated as a hard ceiling that gates any further loosening of the auto-remediation threshold.

```mermaid
flowchart TD
    A[Agent reasoning: decides on a remediation action] --> P[Proposed action\ntool + parameters + target]
    P --> S[Authorization service\ndeterministic, outside the LLM]
    S --> D{Reversible, narrow blast radius,\nplaybook proven in shadow mode?}
    D -->|yes| E[Actuator executes autonomously]
    D -->|no| H[Escalate to human approval]
    E --> R[(Rollback / compensating action\nheld ready if wrong)]
    H --> E
    S --> L[(Audit log: proposal + decision + outcome)]
```

### G3. Access-Control Leak in a RAG-Powered Security Co-Pilot

**"A security co-pilot agent with RAG access to internal incident tickets and runbooks starts leaking details from tickets the requesting analyst shouldn't have clearance to see — how do you find and fix this?"**

**Approach:**
- **Objective/metric:** Eliminate the information-disclosure vulnerability at its actual root cause — this is a retrieval/authorization-architecture bug, not a prompting problem, so "add an instruction telling the model not to do that" is not an acceptable fix even if it happens to reduce the symptom.
- **Clarifying questions:** Does the co-pilot use one shared vector index across all clearance levels/tenants, or separate indices? Is per-document/per-ticket ACL metadata (sensitivity, department, customer scope) even attached at index time? How was the leak discovered — a specific analyst report, or found through testing?
- **Data & features:** The requesting analyst's actual resolved entitlements (role, clearance, case/tenant assignment) as of query time, and the ACL/sensitivity metadata attached to each indexed ticket/runbook chunk — both need to exist as structured, queryable fields, not just be implied by document content.
- **Approach (diagnose → fix):** Diagnose first: vector similarity search has no inherent concept of permissions, so a semantically relevant but unauthorized ticket gets retrieved purely because it's a good textual match, and once its content is sitting in the model's context, the LLM cannot be reliably trusted to notice and refuse to use it — the leak has already happened at the retrieval layer by the time any generation-time safeguard runs. Fix: enforce **access-control-aware retrieval** — resolve the requester's entitlements before the retrieval call and apply them as a *pre-filter* on the vector search itself (e.g., `acl_group IN [analyst's groups] AND clearance <= analyst's level`), so unauthorized tickets are never candidates at all, not retrieved-then-hidden; post-filtering after the top-k comes back is both leakier (unauthorized content briefly exists in the retrieved set and any logs/traces) and worse for recall. For the highest-sensitivity ticket categories, use fully separate per-clearance/per-tenant indices (no shared search space to leak across, by construction); for everything else, a shared index with a mandatory ACL filter applied on every single retrieval code path, treated as a security invariant to test explicitly.
- **Detection in practice:** Structured audit logging of every retrieval (which chunks were returned to which requester, at what clearance) to catch this retroactively; permission-boundary test cases baked into the regression/golden-set suite so a deliberately-crafted low-clearance query probing high-clearance content is checked before every deploy of a prompt, model, or index change; and periodic red-team/adversarial testing specifically targeting this boundary, since a passing golden set today doesn't guarantee a new document/index update doesn't reopen the gap.
- **Risks/tradeoffs & validate impact:** No single layer should be trusted alone — pair the retrieval-layer pre-filter with an independent second check (an authorization step validating the final answer's cited sources against the requester's entitlements) as defense-in-depth, so one missed filter on one code path doesn't equal a full breach; re-run the expanded permission-boundary golden set after the fix, and keep adversarial probing of this boundary as an ongoing practice, not a one-time remediation.
...
| G1 | SOC alert fatigue, multi-agent triage | A Blue-style triage agent funnels alerts (auto-close / auto-escalate / human-review-the-middle); never optimize raw alert-volume reduction without missed-threat rate as a hard ceiling. |
| G2 | Authorizing an autonomous remediation agent | The model only proposes — a deterministic authorization service gates every action by reversibility/blast radius; least-privilege agent credentials bound the confused-deputy risk, and every autonomous action needs a rollback path. |
| G3 | RAG security co-pilot clearance leak | Enforce access control as a pre-filter on retrieval itself, never trust the LLM to self-censor an already-retrieved chunk; pair with per-clearance index isolation, audit logging, and red-teaming as defense-in-depth. |
