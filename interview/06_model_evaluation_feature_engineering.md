...
3. [Ranking as an ML Problem Type](#3-ranking-as-an-ml-problem-type)
4. [Calibration](#4-calibration)
5. [Cross-Validation](#5-cross-validation)
6. [Handling Imbalanced Datasets](#6-handling-imbalanced-datasets)
7. [Feature Engineering](#7-feature-engineering)
8. [Explainability — SHAP, LIME, Permutation Importance, PDP](#8-explainability--shap-lime-permutation-importance-pdp)
9. [Popular Questions — Full Answers](#9-popular-questions--full-answers)
...
## 3. Ranking as an ML Problem Type

### 3.1 Why Ranking Is Its Own Problem Type, Not Classification or Regression

Everything in Sections 1 and 2 evaluates a model's ability to produce a **correct absolute output** for each individual example — the right class label, or the right numeric value. **Ranking** (also called **learning to rank**, LTR) asks a fundamentally different question: given a query (a search query, a user, a context) and a set of candidate items, produce the **correct relative order** of those items. Getting every item's absolute score slightly wrong is fine if the *order* they induce is right; getting one item's absolute score badly wrong is catastrophic if it causes an irrelevant item to be ranked above a relevant one.

This distinction shows up everywhere in industry: search engines ranking documents for a query, recommender systems ranking items for a user's feed, ads systems ranking candidate ads by expected value, and rerankers in RAG pipelines ranking retrieved chunks by relevance to a question. In all these settings:

- There is a **grouping structure** (query/user/session) — items are only ever compared *within* their own group, never across groups. A document scoring 0.9 for query A and a document scoring 0.4 for query B says nothing about their relative order, because they were never candidates for the same slot.
- The **label is inherently relative or ordinal** — often graded relevance (0 = irrelevant, 1 = somewhat relevant, 2 = highly relevant) or an implicit signal (clicked vs. not clicked, purchased vs. not) rather than a clean regression target.
- What's actually consumed downstream is a **sorted list**, so the natural loss to optimize is one that rewards correct *ordering*, not one that rewards correct *magnitude* of individual scores.

This motivates three families of approaches — pointwise, pairwise, and listwise — that differ in exactly what unit of prediction the loss function operates on.

### 3.2 Pointwise Approach

Treat ranking as an ordinary supervised learning problem on **individual (query, item) pairs**: predict a relevance score $\hat s_i$ for each item independently (via regression on a graded relevance label, or classification/probability estimation on a binary relevance label), then **sort candidates within each query by predicted score** at inference time.

- Simple to implement and train — reuses any standard regression/classification pipeline, loss function, and infrastructure (e.g., plain MSE or logistic loss per item).
- **Doesn't directly optimize order quality.** The loss treats every item's error independently and symmetrically — it has no notion that item A and item B belong to the same query and must be correctly ordered relative to each other. A model can achieve a low pointwise loss while still getting the *relative* order of a specific query's top candidates wrong, because a uniformly-scaled or slightly miscalibrated set of scores can still preserve low absolute error yet flip pairs near the decision-relevant boundary.
- Also doesn't naturally account for **position** — a mistake at rank 1 (the item the user actually sees first) is treated identically to a mistake at rank 50, even though only the former usually matters for the metrics that matter (Section 3.5).

### 3.3 Pairwise Approach — RankNet

Instead of predicting an absolute score per item, train on **relative preferences between pairs of items from the same query**: given a pair $(i, j)$ where $i$ is known to be more relevant than $j$ ($i \succ j$), the loss should push the model to score $i$ higher than $j$ — the *sign* and *ordering* of the score difference is what's being learned, not the absolute values.

**RankNet** (Burges et al., 2005) is the canonical formulation. Given the model's raw scores $s_i = f(x_i)$ and $s_j = f(x_j)$ for two items from the same query, the modeled probability that $i$ should rank above $j$ is a sigmoid of the score difference:

$$
P(i \succ j) = \sigma(s_i - s_j) = \frac{1}{1 + e^{-(s_i - s_j)}}
$$

This is compared against the true pairwise label $\bar P_{ij} \in \{0, 0.5, 1\}$ (1 if $i$ is truly more relevant than $j$, 0 if less, 0.5 if tied/unknown) using **binary cross-entropy**:

$$
\mathcal{L}_{ij} = -\bar P_{ij}\log P(i \succ j) - (1-\bar P_{ij})\log\big(1 - P(i \succ j)\big)
$$

- Notice this is exactly logistic regression's loss, just applied to a **score difference** between two items rather than to a single item's raw score — the model never needs to learn a "correct absolute value," only a scoring function whose *differences*, in aggregate across all training pairs, respect the observed preference ordering.
- Gradients push $s_i$ up and $s_j$ down whenever the pair is currently mis-ordered (or under-separated), with the push proportional to how wrong the current pairwise probability is — pairs that are already correctly and confidently ordered contribute little gradient, focusing training signal on ambiguous/incorrectly-ordered pairs.
- **Still a limitation**: every violated pair contributes to the loss with equal weight regardless of *where in the final ranked list* that pair sits. Swapping two low-relevance items near the bottom of a 50-item list counts the same as swapping the top two items — even though only the latter usually affects real ranking-quality metrics like NDCG@10. This is exactly the gap listwise methods close.

### 3.4 Listwise Approach — LambdaMART

**Listwise** methods optimize a loss defined over the **entire ranked list for a query at once**, directly targeting a ranking metric such as NDCG, rather than decomposing into independent points or pairs.

The complication: ranking metrics like NDCG are computed from a list's **sort order** (which item ended up in which position), so they're **non-smooth / non-differentiable** in the model's raw scores — a tiny change in a score can suddenly swap two items' ranks and cause a discontinuous jump in NDCG, with zero gradient everywhere else. You can't directly backpropagate through "sort, then compute NDCG."

**LambdaMART** — the industry-standard listwise-flavored approach — sidesteps this cleverly rather than solving it head-on:

- It's a **gradient-boosted decision tree model** (ties directly to the boosting mechanics in `04_trees_ensembles_boosting.md` — same additive-trees-fit-to-pseudo-residuals framework, just with a different definition of "residual").
- Instead of a single differentiable list-level loss, LambdaMART keeps the **pairwise RankNet machinery** (comparing pairs within a query) but **reweights each pair's gradient by $|\Delta \text{NDCG}|$** — the magnitude of change in the target ranking metric (typically NDCG) that would result from **swapping that specific pair's positions** in the current ranking. These reweighted gradients are called **lambda gradients**:
  $$
  \lambda_{ij} = \frac{\partial \mathcal{L}_{ij}}{\partial s_i} \cdot |\Delta \text{NDCG}_{ij}|
  $$
  i.e., take the ordinary RankNet pairwise gradient and scale it by how much swapping $i$ and $j$ would move NDCG for that query's list.
- Effect: a pair sitting at ranks 1 and 2 of a highly-relevant query gets a **much larger gradient** than a pair sitting at ranks 40 and 41, because swapping the former changes NDCG a lot (position 1 is heavily weighted by NDCG's $\log_2(i+1)$ discount) while swapping the latter barely moves it. The model is therefore pushed hardest to get the *top* of each list right — exactly what ranking metrics reward — without ever needing an explicit differentiable formula for NDCG itself.
- This is why LambdaMART is described as **"pairwise training, listwise-metric-aware"**: the mechanics operate pair-by-pair (compatible with standard gradient boosting, which fits trees to per-example gradients), but the *magnitude* of each pair's gradient is informed by its impact on a whole-list metric — giving most of listwise's benefit without listwise's non-differentiability problem.
- LambdaMART consistently wins or ties top learning-to-rank benchmarks (it won the Yahoo! Learning to Rank Challenge) and is the basis for ranking components in several production search/recommendation stacks, precisely because gradient-boosted trees already handle heterogeneous, non-linear, tabular-style features well, and the lambda-gradient trick bolts metric-awareness onto that framework without needing a separate architecture.

### 3.5 Ranking Evaluation Metrics

*(These are the same retrieval/ranking metrics introduced for search and RAG evaluation in `12_rag_agents_llm_systems.md` — restated here briefly since ranking quality is this section's direct topic; see that file for the fuller retrieval-specific discussion of when to pick which.)*

**NDCG (Normalized Discounted Cumulative Gain)** is the standard metric precisely because it matches what listwise methods like LambdaMART optimize for. Discounted Cumulative Gain accumulates graded relevance down the list, discounted logarithmically by position so that relevant items ranked lower contribute less:

$$
DCG@k = \sum_{i=1}^{k} \frac{rel_i}{\log_2(i+1)}
$$

(A common graded-relevance variant instead uses $\frac{2^{rel_i}-1}{\log_2(i+1)}$ to more heavily emphasize highly-relevant items — the two forms are equivalent when relevance is binary.) **NDCG** normalizes DCG by the DCG of the *ideal* ranking (items sorted by true relevance, $IDCG@k$), producing a score in $[0,1]$ that's comparable across queries with different numbers of relevant items:

$$
NDCG@k = \frac{DCG@k}{IDCG@k}
$$

Two other metrics complete the standard ranking toolkit:

- **MRR (Mean Reciprocal Rank)** — $\frac{1}{|Q|}\sum_i \frac{1}{\text{rank}_i}$, the average reciprocal rank of the *first* relevant item per query; best when there's typically one right answer and you care how fast it's surfaced.
- **Precision@k / Recall@k** — the fraction of the top-$k$ that's relevant (Precision@k) and the fraction of all relevant items captured within the top-$k$ (Recall@k); simple, binary-relevance, position-agnostic-within-$k$ checks of coverage rather than full ordering quality.

Only NDCG (of these three) is sensitive to the *exact order within* the top-$k$, which is why it — not Precision@k or MRR — is the metric listwise LTR losses are built to approximate.

### 3.6 Comparison Table

| Approach | Unit of prediction | What it optimizes | Example algorithm | Typical use case |
|---|---|---|---|---|
| Pointwise | Single (query, item) score | Per-item regression/classification loss (MSE, logloss) | Logistic/linear regression on relevance labels; GBM regressor on click/relevance | Quick baseline; relevance scoring where absolute calibration also matters |
| Pairwise | Pair of items from the same query | Correct relative order of item pairs (cross-entropy on score difference) | **RankNet**, RankSVM | Search ranking, ads ranking — when you have reliable pairwise/preference judgments |
| Listwise | Entire ranked list for a query | A whole-list ranking metric (NDCG) directly, or a metric-aware proxy | **LambdaMART**, ListNet, LambdaRank | Production search & recommendation ranking, learning-to-rank in ads — state-of-the-art results, benchmark-winning |

**Interview angle:**

> **Q: Why would you choose a pairwise or listwise ranking loss over just training a regressor on relevance scores?**
> A: A pointwise regressor minimizes absolute error on individual item scores, but nothing in that objective directly rewards getting the *relative order* of items within a query right — which is the only thing that actually matters once you sort and show a ranked list. Two items can each have a "good enough" absolute error yet still end up swapped in the final ranking, and a pointwise loss has no way to penalize that swap specifically. A pairwise loss like RankNet instead trains directly on relative comparisons — $P(i \succ j) = \sigma(s_i - s_j)$ compared against the true preference via cross-entropy — so the gradient explicitly pushes toward correct ordering rather than correct magnitude. Listwise methods go one step further: LambdaMART keeps RankNet's pairwise gradient computation (since a differentiable listwise NDCG loss isn't tractable directly) but rescales each pair's gradient by how much swapping that pair would actually change NDCG for the whole list — so the model is pushed hardest on the pairs that matter most for the metric actually used to judge the ranking, typically the pairs near the top of the list. In practice, listwise (LambdaMART) is the standard choice in production search/recommendation ranking because it's the closest thing to directly optimizing the metric (NDCG) the business actually cares about, while pointwise is reserved for quick baselines or cases where the raw relevance score itself (not just the order) is separately useful downstream.
...
### 4.1 Why Tree Ensembles and Boosted Models Are Often Poorly Calibrated
...
### 5.4 Why Standard K-Fold Is Invalid for Time Series
...
### 5.5 Walk-Forward Validation: Expanding Window vs Sliding Window
...
### 6.2 SMOTE (Synthetic Minority Oversampling Technique)
...
### 7.2 Scaling: Standardization vs Normalization
...
### 7.4 Missing Data Imputation Strategies
...
### 7.5 Outlier Detection and Treatment
...
## 8. Explainability — SHAP, LIME, Permutation Importance, PDP

### 8.1 SHAP: Foundations in Cooperative Game Theory
...
### 8.2 The Shapley Value Formula, Fully Derived
...
### 8.3 The Three Axiomatic Guarantees
...
### 8.4 TreeSHAP: Making Exact Shapley Values Tractable
...
### 8.5 SHAP vs Plain Tree Feature Importance (Gain / Split-Count)
...
### 8.6 LIME (Local Interpretable Model-agnostic Explanations)
...
### 8.8 Partial Dependence Plots (PDP) and ICE
...
- **Ranking** is its own ML problem type — goal is correct *relative order* within a query/user group, not a correct absolute label/value. **Pointwise** = regress/classify each item independently, then sort (simple, doesn't optimize order). **Pairwise** = train on relative preferences between item pairs; **RankNet** loss: $P(i \succ j) = \sigma(s_i - s_j)$, cross-entropy vs. true pairwise label. **Listwise** = optimize a whole-list metric directly; **LambdaMART** = gradient-boosted trees trained with RankNet pairwise gradients rescaled by $|\Delta \text{NDCG}|$ from swapping that pair ("lambda gradients") — pairwise mechanics, listwise-metric-aware. Eval: **NDCG** $=DCG@k/IDCG@k$ where $DCG@k=\sum_{i=1}^k rel_i/\log_2(i+1)$ (position-sensitive, order-aware); **MRR** (rank of first relevant item); **Precision@k/Recall@k** (coverage, position-agnostic within $k$).
