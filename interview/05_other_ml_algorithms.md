# Other ML Algorithms: SVM, k-NN, Naive Bayes, Clustering, Anomaly Detection & Dimensionality Reduction

This file covers the classical, non-neural-network ML algorithms that remain heavily interview-tested for applied ML/forecasting roles: SVMs and the kernel trick, k-NN and the curse of dimensionality, Naive Bayes, the major clustering families, anomaly detection (isolation forest, one-class SVM, autoencoder-based), and dimensionality reduction (PCA/t-SNE/UMAP/feature selection). Each section derives the core math, walks through the algorithm mechanics with a diagram where useful, and closes with realistic interview Q&A.
...
5. [Anomaly Detection](#anomaly-detection)
   - [Framing: Supervised vs. Unsupervised vs. Semi-Supervised](#framing-supervised-vs-unsupervised-vs-semi-supervised)
...
7. [Popular Questions — Full Answers](#popular-questions--full-answers)
...
### Framing: Supervised vs. Unsupervised vs. Semi-Supervised

**Anomaly detection** (also called outlier detection or novelty detection) is the task of identifying rare observations that deviate substantially from the distribution the bulk of the data follows — fraudulent transactions among legitimate ones, a failing sensor reading among healthy ones, a defective part on an assembly line, an intrusion in network traffic. The core assumption underlying almost every method is that "normal" behavior is common and reasonably well-structured (dense, clustered, low-dimensional, reconstructible), while anomalies are rare and structurally different — they don't fit whatever pattern the normal data follows.

How you approach the problem depends heavily on what labels are available, and in practice **labels are the exception, not the rule**:

- **Supervised**: you have confirmed labels for both normal and anomalous examples, so you can train an ordinary classifier (logistic regression, gradient boosting, etc.) directly on "is this anomalous?" This is the least common framing in practice — confirmed anomalies are by definition rare, so the positive class is tiny and expensive to label, and a classifier trained only on *known* anomaly types has no mechanism for catching a genuinely novel failure mode it never saw in training.
- **Unsupervised**: no labels at all. The method has to infer "normalcy" purely from the structure of the (unlabeled, presumed mostly-normal) data itself, then flag points that don't fit that structure. Isolation forest is the canonical example — it makes no assumption about which points are anomalous ahead of time.
- **Semi-supervised (one-class)**: you train exclusively (or almost exclusively) on data known/assumed to be normal — no labeled anomalies required, just a clean or mostly-clean training set — and the model learns a boundary or reconstruction of "normal," flagging anything at inference time that falls outside it. One-Class SVM and autoencoder-based detection are typically deployed this way: train only on normal operating data, then treat deviation from what the model learned as the anomaly signal.

Even when labels do exist, this is an extreme version of the **class-imbalance** problem covered in file 06 — confirmed anomalies are routinely well under 1% of the data, so accuracy is meaningless, and techniques like resampling, class weighting, and evaluating with precision/recall or PR-AUC rather than accuracy matter even more here than in typical imbalanced classification. This scarcity of reliable positive labels is precisely why unsupervised and semi-supervised approaches dominate real-world anomaly detection: waiting for a large, representative labeled anomaly set is often simply infeasible, and by the time you have one, the anomaly patterns may have already shifted.

### Isolation Forest

**Core intuition**: anomalies are "few and different" — they sit in low-density regions, far from the bulk of normal points. If you build a tree by repeatedly picking a random feature and a random split threshold, an anomalous point tends to get **isolated (separated into its own leaf) after very few splits**, because there's a lot of empty space around it and few other points to keep sharing a partition with it. A normal point, sitting in a dense region surrounded by many similar points, requires many more splits before it's finally separated from all its neighbors.

**Algorithm**:
1. Build an ensemble of **isolation trees (iTrees)**. Each tree is built on a random subsample of the data (subsampling — typically 256 points — is enough, and it keeps the trees shallow and fast, unlike bagging in random forests which usually samples close to the full dataset size).
2. At each node, pick a feature uniformly at random, then a split threshold uniformly at random between that feature's min and max value in the current node's data.
3. Recurse until each point is isolated in its own leaf, or a height limit is reached.
4. For a point $x$, its **path length** $h(x)$ in one tree is the number of edges traversed from the root to the leaf containing $x$.
5. Average $h(x)$ across all trees in the forest to get $E(h(x))$ — shorter average path length means the point was easy to isolate, i.e. more anomalous.

**Anomaly score**:

$$s(x, n) = 2^{-\frac{E(h(x))}{c(n)}}$$

where $E(h(x))$ is the average path length to isolate $x$ across the trees, and $c(n)$ is a normalization constant — the average path length of an *unsuccessful search* in a Binary Search Tree built from $n$ points, used because path length grows with $n$ and needs to be put on a comparable scale across dataset sizes:

$$c(n) = 2H(n-1) - \frac{2(n-1)}{n}, \qquad H(i) \approx \ln(i) + 0.5772156649\ldots \text{ (harmonic number, Euler–Mascheroni constant)}$$

**Interpreting the score at the extremes**:
- $E(h(x)) \to 0$ (isolated almost immediately) $\Rightarrow s \to 1$ — strong anomaly.
- $E(h(x)) \to c(n)$ (isolated at roughly the "typical" BST depth) $\Rightarrow s \to 0.5$ — ambiguous/no strong signal either way.
- $E(h(x)) \to n-1$ (needs almost the full depth of the tree to isolate) $\Rightarrow s \to 0$ — normal point, deeply embedded in dense regions.

In practice, points with $s$ close to 1 (often thresholded around 0.6–0.7+, tuned per dataset) are flagged as anomalies; if *no* point in the dataset scores meaningfully above 0.5, that's itself a signal the dataset likely has no distinct anomalies at all.

```mermaid
flowchart TD
    A[Random subsample of data] --> B[Build iTree: at each node,\n pick random feature + random threshold]
    B --> C[Recurse until point isolated\n or height limit reached]
    C --> D[Repeat across many trees\n to build the forest]
    D --> E[For point x: average path length\n E(h(x)) across all trees]
    E --> F["Anomaly score s(x,n) = 2^(-E(h(x))/c(n))"]
    F --> G{s close to 1?}
    G -->|Yes| H[Flag as anomaly:\n isolated in very few splits]
    G -->|No, s <= ~0.5| I[Normal: needed deep splits\n to isolate]
```

Because isolation forest never computes a distance or a density estimate — it only asks "which random split did this point fall on" — it sidesteps the curse of dimensionality that cripples distance-based outlier methods (like distance-to-k-th-neighbor approaches), and scales comfortably to high-dimensional, large datasets: building each tree is $O(n \log n)$ and scoring is $O(\log n)$ per point per tree.

**Interview angle:**

- **Q: Why does isolation forest scale better to high-dimensional data than a distance-based anomaly detector?**
  A: Distance/density-based methods (e.g., k-th nearest neighbor distance, LOF) suffer from the curse of dimensionality — in high dimensions, pairwise distances concentrate and stop being discriminative, so "how far is this point from its neighbors" becomes a noisy signal. Isolation forest never computes a distance at all: it only asks which side of a random axis-aligned split a point falls on, repeatedly, so its cost and effectiveness don't degrade the same way as $d$ grows. It also doesn't need to model the full joint density of the normal class, which itself becomes intractable in high dimensions.

- **Q: Why does isolation forest subsample a small number of points (e.g. 256) per tree rather than using the full dataset, unlike a typical random forest?**
  A: The goal is to isolate anomalies via short paths, and large sample sizes actually work against that — with more points, the average path length needed to isolate any single point (including a true anomaly) grows, diluting the separation in path length between normal and anomalous points ("swamping" and "masking" effects, where too many points crowd around anomalies or nearby anomalies bury each other's signal). A small subsample keeps trees shallow and keeps the anomalies' shorter relative path length clearly distinguishable, while also making each tree cheap to build — this is a case where more data per tree actively hurts the algorithm's core mechanism, unlike bagging for variance reduction in a standard random forest.

### One-Class SVM

**Core intuition**: rather than separating two classes (as in standard SVM), One-Class SVM learns a boundary that separates the bulk of the "normal" data from the origin in a (typically kernel-mapped) feature space, with maximum margin — treating anything falling outside that boundary as an outlier. Equivalently, it finds the smallest region (in feature space) that still contains most of the data.

Using the **kernel trick** already covered in the SVM section above (same $K(x,x') = \phi(x)\cdot\phi(x')$ machinery — RBF is the standard choice here too), the optimization is:

$$\min_{w,\rho,\xi}\; \frac{1}{2}\|w\|^2 + \frac{1}{\nu n}\sum_{i=1}^n \xi_i - \rho \quad \text{subject to} \quad w\cdot\phi(x_i) \geq \rho - \xi_i,\;\; \xi_i \geq 0$$

The decision function is $f(x) = \text{sign}(w\cdot\phi(x) - \rho)$: points on the "normal" side of the learned hyperplane (offset $\rho$ from the origin) score positive, points pushed to the other side by too little support score negative and are flagged as anomalies.

**The $\nu$ hyperparameter** (in $[0,1]$) plays a dual role, both bounds tight in the limit:
- An **upper bound** on the fraction of training points allowed to be margin errors / treated as outliers.
- A **lower bound** on the fraction of training points that end up as support vectors.

So $\nu$ is effectively your prior belief about "roughly what fraction of my training data is contamination" — set it too low and the model tries to enclose every point including true outliers (overly permissive boundary); set it too high and it aggressively excludes points that are actually normal (overly tight boundary, more false positives).

Because it relies on the same kernel machinery as standard SVM, One-Class SVM inherits the same practical considerations: features should be scaled before use (kernel distances are scale-sensitive, exactly as in the two-class case), and kernel methods generally scale poorly to very large $n$ (naive training is roughly $O(n^2)$–$O(n^3)$), which limits its use on very large datasets relative to isolation forest.

**Interview angle:**

- **Q: What does the $\nu$ parameter in One-Class SVM actually control, and how would you set it?**
  A: $\nu$ simultaneously upper-bounds the fraction of training points allowed to be treated as outliers/margin errors and lower-bounds the fraction of points that become support vectors. In practice, I'd set it to a rough estimate of the expected contamination rate in the training data — e.g., if I believe roughly 2% of transactions in my "normal" training set might actually be undetected fraud, $\nu \approx 0.02$ is a reasonable starting point, then tune it via validation against any labeled anomalies I do have, or against downstream business cost of false positives vs. false negatives.

### Autoencoder-Based Anomaly Detection

**Core intuition**: train an autoencoder — an encoder-decoder neural network with a compressive bottleneck — to reconstruct **only normal data** as accurately as possible: $x \xrightarrow{\text{encoder}} z \xrightarrow{\text{decoder}} \hat{x}$, where $z$ is a lower-dimensional latent representation. Because the network is trained exclusively (or almost exclusively) on normal examples, it learns to compress and reconstruct *only the patterns present in that normal data's manifold*. When it's shown an anomalous input at inference time — one whose structure wasn't represented in training — the encoder-decoder pipeline reconstructs it poorly, since it never learned the right latent structure to represent that kind of input efficiently.

**Anomaly score** — the reconstruction error itself:

$$\text{score}(x) = \|x - \hat{x}\|^2$$

(mean squared error between the input and its reconstruction; other reconstruction losses, e.g. binary cross-entropy for normalized/pixel-like data, are also used).

**Thresholding**: a point is flagged as anomalous if its reconstruction error exceeds some threshold, commonly chosen as a high percentile (e.g. the 95th or 99th percentile) of reconstruction error observed on a held-out set of known-normal data, or as a fixed number of standard deviations above the mean normal reconstruction error. As with any threshold-based detector, this is a precision/recall tradeoff dial — a stricter (lower) threshold catches more anomalies at the cost of more false positives, and vice versa.

```mermaid
flowchart LR
    A["Normal training data x"] --> B["Encoder: compress to\n latent z (bottleneck)"]
    B --> C["Decoder: reconstruct x_hat from z"]
    C --> D["Train to minimize\n ||x - x_hat||^2 on normal data only"]
    E["New input x (at inference)"] --> F[Pass through trained\n encoder-decoder]
    F --> G["Reconstruction error\n ||x - x_hat||^2"]
    G --> H{Error > threshold?}
    H -->|Yes| I[Flag as anomaly:\n model never learned to\n reconstruct this pattern]
    H -->|No| J[Normal: reconstructs cleanly]
```

**When this approach is preferred**: autoencoder-based detection shines on **complex, high-dimensional data whose "normal" manifold is itself highly non-linear** — images, audio, sensor/time-series streams (often with recurrent or convolutional encoder/decoder architectures, e.g. LSTM-autoencoders for sequences) — where a linear method (PCA-based reconstruction error, the same idea but with a linear subspace) underfits the true shape of normal data, and a tree-based method like isolation forest, which splits on individual features independently, doesn't explicitly model the joint, correlated structure across many raw input dimensions (e.g. spatial correlation between neighboring pixels). The cost is that autoencoders need considerably more normal training data and compute than isolation forest or One-Class SVM, and are more opaque to tune and validate.

**Interview angle:**

- **Q: When would you reach for isolation forest vs. an autoencoder for anomaly detection?**
  A: Isolation forest is my default first pass for tabular, moderate-dimensional data — it's fast to train, needs no labels or normal-only training set, scales well, and requires little tuning, making it a strong baseline for things like transaction-level fraud features. I'd reach for an autoencoder when the data is complex and high-dimensional with a genuinely non-linear underlying structure — images, sensor waveforms, time-series windows — where the "normal" pattern isn't well captured by axis-aligned splits or a linear subspace, and where I have enough clean normal data to actually train a network. In between, One-Class SVM is a reasonable choice for moderate-sized, moderate-dimensional data where I want an explicit, kernel-based margin around "normal" and have a decent prior on the contamination rate ($\nu$), but it doesn't scale as well as isolation forest and doesn't capture complex non-linear manifolds as well as an autoencoder. In practice I'd often run isolation forest first as a cheap baseline regardless, since it needs the least setup, and escalate to an autoencoder only if the underlying data's complexity clearly demands it.

### Method Comparison Table

| Method | Assumption | Scales to high-dim? | Needs labels? | Typical use case |
|---|---|---|---|---|
| **Isolation Forest** | Anomalies are few & different → isolated in fewer random splits; no distributional assumption | Yes — no distance/density computation, scales well to high $d$ and large $n$ | No (unsupervised) | Tabular data, fraud detection, general-purpose fast baseline |
| **One-Class SVM** | Normal data occupies a single dense region separable from the origin with max margin in (kernel) feature space | Moderate — kernel computations are $O(n^2)$–$O(n^3)$ and RBF-kernel distances suffer the same curse of dimensionality as other kernel/distance methods | No (unsupervised/semi-supervised), but $\nu$ needs a rough prior on contamination rate | Novelty detection on small-to-moderate, moderate-dimensional data |
| **Autoencoder** | Normal data lies on a learnable, possibly non-linear low-dimensional manifold; network can only compress/reconstruct in-distribution patterns well | Yes — most valuable precisely in high dimensions (images, sensor streams) where linear/tree methods underfit | No (self-supervised — needs only normal data to train) | Images, audio, sensor/time-series data, complex high-dim data with non-linear structure |
...
- **Anomaly detection framings**: supervised (rare, needs labeled anomalies), unsupervised (infer normalcy from unlabeled data — isolation forest), semi-supervised/one-class (train on normal-only data — One-Class SVM, autoencoder). Confirmed anomalies are almost always <1% of data — extreme class imbalance.
- **Isolation Forest**: random splits isolate "few & different" points faster. Score $s(x,n)=2^{-E(h(x))/c(n)}$, $c(n)=2H(n-1)-\frac{2(n-1)}{n}$; $s\to1$ = anomaly (isolated fast), $s\to0.5$ = ambiguous, $s\to0$ = normal (deep to isolate). No distance/density computation → scales well to high dims.
- **One-Class SVM**: max-margin boundary separating data from the origin in kernel feature space (same kernel trick as SVM). $\nu$ = upper bound on outlier fraction / lower bound on support-vector fraction.
- **Autoencoder anomaly detection**: train encoder-decoder to reconstruct normal data only; score = reconstruction error $\|x-\hat x\|^2$; threshold at a high percentile of normal-data error. Preferred for complex, high-dim, non-linear-manifold data (images, sensor/time-series) where PCA/isolation forest underfit.
