...
This file is different from the rest of the kit. Files 01-16 teach the
...
5. **Night-before-interview:** Do one fast full pass of the whole deck (all
   ~270+ cards) as a confidence check, not a learning session. You should be
...
## 8. SQL, PySpark, dbt & Data Engineering (→ `08_sql_pyspark_dbt_data_engineering.md`)
...
## 9. MLOps & Cloud Deployment (→ `09_mlops_cloud_deployment.md`)
...
## 10. NLP & Deep Learning Fundamentals (→ `10_nlp_and_deep_learning_fundamentals.md`)
...
## 11. GenAI, LLMs & Transformers (→ `11_genai_llms_transformers.md`)
...
## 12. RAG, Agents & LLM Systems (→ `12_rag_agents_llm_systems.md`)
...
## 13. ML System Design (→ `13_system_design_ml.md`)
...
## 14. Agentic Systems Architecture & Evaluation (→ `14_agentic_systems_architecture_and_evaluation.md`)

<details>
<summary><b>Q: What's the key architectural axis that separates a "workflow-driven" agent from a fully "autonomous/LLM-driven" one, and why do production security agents default to the former?</b></summary>

The axis is deterministic vs. LLM-driven orchestration — whether the sequence of steps and branching conditions are fixed in code ahead of time, or decided by the LLM at runtime. Production security agents default to deterministic orchestration because it bounds the blast radius of a bad LLM output to whatever one step controls, keeps the system auditable (a fixed set of legal transitions to reason about), and is easy to unit-test — the LLM is used only inside bounded steps (summarizing, classifying, drafting), never to decide which step runs next or whether to skip an approval gate.

</details>

<details>
<summary><b>Q: Task decomposition vs. hierarchical planning — what's the difference?</b></summary>

Task decomposition is the basic move of breaking a high-level goal into tractable sub-tasks. Hierarchical planning goes further by planning at multiple levels of abstraction simultaneously — a rarely-changing, human-auditable strategic plan ("this looks like credential compromise — investigate lateral movement, then contain") is decomposed into a tactical plan that's regenerated or adapted per-incident.

</details>

<details>
<summary><b>Q: ReAct vs. plan-and-execute as agent execution-loop architectures — what's the core tradeoff?</b></summary>

ReAct interleaves reasoning and acting one step at a time with no upfront plan — adaptive to new information, but the full action sequence is opaque until it's already run. Plan-and-execute produces a complete multi-step plan upfront that an executor carries out, replanning only on deviation — the plan is human-reviewable/auditable before any action executes, at the cost of wasted planning if early results change everything.

</details>

<details>
<summary><b>Q: What does tree/graph search (MCTS-style) add to agent planning that ReAct and plan-and-execute don't have?</b></summary>

The ability to explore multiple candidate action sequences before committing, with backtracking — if a branch's early steps dead-end, the search abandons it and resumes from an earlier node rather than staying stuck on one failing trajectory. It's expensive, so it's reserved for high-value, high-ambiguity tasks where the first hypothesis genuinely isn't clear from the initial signal alone.

</details>

<details>
<summary><b>Q: What's the difference between a tool call "succeeding" and a verification loop passing?</b></summary>

A tool call succeeding just means it executed without a transport/API-level error. A verification loop checks something stronger: did the content of the response actually satisfy the sub-goal — a log query that returns zero rows because of a timezone bug is a successful call and a failed verification, and without an explicit check the agent would misread "the query was wrong" as "confirmed: no suspicious activity."

</details>

<details>
<summary><b>Q: Rank short-term, episodic, and semantic memory by durability, and describe how episodic memory typically becomes semantic memory.</b></summary>

Short-term/context memory lasts only as long as it's in the context window; episodic memory is durable and indexed by specific past events ("what happened when"); semantic memory is durable and distilled, decoupled from any one originating episode. Episodic memory becomes semantic memory through summarization/compaction — many raw episodic entries get collapsed into a smaller number of generalized facts/rules.

</details>

<details>
<summary><b>Q: What is memory contamination, and how does it connect to memory poisoning as an attack?</b></summary>

Memory contamination is irrelevant, stale, or adversarially-injected content polluting an agent's memory store and biasing future reasoning — e.g., a false "confirmed benign" fact written during one investigation gets inherited by every future investigation that retrieves it. Memory poisoning is the deliberate version: an adversary engineers inputs so the agent writes a false, exploit-enabling fact into durable memory — which is why persistent memory needs provenance tracking and expiry, not blind trust-on-write.

</details>

<details>
<summary><b>Q: Name the three standard memory pruning strategies.</b></summary>

Summarization/compaction (collapse many episodic entries into distilled semantic ones), recency + relevance-weighted forgetting (decay retrieval priority like a cache-eviction policy scored by recency and usage), and explicit TTLs (hard-expire facts with a known shelf life, like IP reputation).

</details>

<details>
<summary><b>Q: Supervisor-worker coordination — what are the three distinct responsibilities a supervisor actually has to handle well?</b></summary>

Routing (deciding which worker(s) handle a sub-task), aggregation (combining worker outputs into a coherent result), and arbitration (resolving conflicts between workers or deciding a worker's output isn't trustworthy enough to use as-is). Many implementations only do routing well and treat aggregation/arbitration as an afterthought — which is often where multi-agent systems actually fail, naively concatenating outputs that should have been flagged as conflicting.

</details>

<details>
<summary><b>Q: Debate vs. voting/ensemble multi-agent patterns — what's each one actually good for?</b></summary>

Debate assigns agents opposing positions that argue and rebut over rounds, with a judge deciding — valuable because structured disagreement surfaces uncertainty a single confident answer would hide, useful for genuinely ambiguous calls. Voting/ensemble has agents independently produce answers to the *same* question, aggregated by majority/weighted vote — reduces variance on a well-defined decision but can't catch a systematic bias shared by all the voters.

</details>

<details>
<summary><b>Q: Blackboard/shared-context vs. peer-to-peer coordination — what's the structural difference, and why is peer-to-peer rare in production security agents?</b></summary>

Blackboard decentralizes agent-to-agent addressing but still centralizes *state* — every agent reads/writes one common, inspectable location. Peer-to-peer decentralizes communication itself — agents message each other directly with no shared board and no guarantee any third party observes the exchange. Peer-to-peer is rare in production security systems because it has no guaranteed termination/convergence, no central place to debug from, and no clean audit trail — exactly the properties a security agent can't sacrifice.

</details>

<details>
<summary><b>Q: Give three distinct reasons adding more agents to a system can make it worse, not better.</b></summary>

Coordination overhead grows roughly combinatorially with agent count; latency stacks additively across sequential dependencies even if each agent is individually fast; and cascading errors occur when one agent's hallucination gets treated as ground truth by every downstream agent that consumes it, compounding rather than being caught. (Also worth naming: token growth from re-passed/regenerated context, duplicated reasoning across agents, and inconsistent state from stale blackboard reads.)

</details>

<details>
<summary><b>Q: Name the five layers of agent evaluation, and explain why you can't stop at just one.</b></summary>

Model-level (accuracy, calibration, hallucination rate) → retrieval-level (recall@k, MRR, NDCG) → agent-level (task completion rate, tool success rate, recovery/escalation rate) → security-agent-level (MTTD/MTTR, unsafe action rate) → production-level (P95/P99 latency, cost/request). A healthy metric at one layer can mask a broken system at another — excellent model-level accuracy doesn't tell you the orchestration layer isn't looping forever.

</details>

<details>
<summary><b>Q: Why is task completion rate insufficient to evaluate a security agent on its own?</b></summary>

It tells you the agent reached *some* goal state but says nothing about whether reaching it was safe, timely, or auditable — a 95% completion rate could still hide that the remaining 5% are the highest-severity incidents, or that some "completed" episodes involved an unsafe remediation action that technically closed the ticket while causing unintended harm.

</details>

<details>
<summary><b>Q: Why doesn't standard software regression testing transfer directly to agent evaluation, and what replaces exact-match assertions?</b></summary>

Standard regression testing assumes determinism — the same input reliably produces the same output, so exact-match is a meaningful signal. An LLM agent given the same input twice can legitimately produce different, equally-correct phrasing or tool-call order, so exact-match would flag all of that as false regressions. Golden/frozen eval sets instead use semantic/fuzzy matching or an LLM-judge rubric, re-run before every deploy, with flakiness managed via retries and majority-vote grading.

</details>

<details>
<summary><b>Q: Walk through the systematic "good yesterday, bad today" debugging checklist for an agent that regressed overnight with no code changes.</b></summary>

Check, in order: model version (silently upgraded behind a fixed alias), prompt version (even one changed elsewhere in a shared library), retrieval/index/embedding/corpus changes, tool/API failure rate or contract changes, latency shifts (timeouts truncating reasoning loops), context size/token distribution growth, routing to a weaker model tier, input/telemetry distribution shift, and evaluator drift (the measurement, not the system, regressed).

</details>

<details>
<summary><b>Q: What is the bulkhead pattern applied to multi-agent systems, and name two concrete mechanisms that implement it?</b></summary>

Bulkheading contains one agent's failure, runaway loop, or resource exhaustion so it can't cascade into the rest of the system — architecting on the assumption some agent will eventually misbehave. Mechanisms: per-agent resource/timeout/token-budget limits (so one looping agent can't starve the others), and circuit breakers scoped per-agent or per-tool rather than one global breaker (so a degraded dependency for one agent doesn't block unrelated agents).

</details>

---

## 15. Security Fundamentals & AI/Agent Security (→ `15_security_fundamentals_and_ai_agent_security.md`)

<details>
<summary><b>Q: State the CIA triad, and classify what a ransomware attack that also exfiltrates data before encrypting it violates.</b></summary>

Confidentiality (unauthorized disclosure), Integrity (unauthorized/undetected tampering), Availability (denial of access). Double-extortion ransomware that exfiltrates data before encrypting it violates both availability (systems/data inaccessible) and confidentiality (data was stolen) simultaneously — a pure integrity violation (subtly altering data without encrypting or exfiltrating anything) would be neither, and arguably more dangerous for being harder to detect.

</details>

<details>
<summary><b>Q: Authentication vs. authorization — give an example of a system with flawless authn but broken authz.</b></summary>

Authentication answers "who are you," authorization answers "what are you allowed to do" — the two are sequential and independent. Classic example: an app requiring a strong password plus MFA to log in, but whose API endpoint trusts a `user_id` parameter from the request body instead of deriving identity from the session — any logged-in user can view another user's data by changing the parameter (an IDOR vulnerability — a broken-authorization failure, not an authentication one).

</details>

<details>
<summary><b>Q: RBAC vs. ABAC — what's each one's core scaling problem, and when do you reach for ABAC?</b></summary>

RBAC assigns permissions to roles statically; its scaling problem is "role explosion" — contextual nuance forces ever-more-specific roles until the model is unmanageable. ABAC evaluates a policy dynamically against subject/resource/action/environment attributes; it scales to nuance without multiplying roles but is harder to audit. Reach for ABAC when the access rule genuinely depends on live context RBAC can't cleanly encode (e.g., "only during business hours, from a managed device").

</details>

<details>
<summary><b>Q: Why does least privilege matter more for an agentic system than for a traditional application?</b></summary>

A traditional application's decision path is written and reviewed by engineers ahead of time, so what can go wrong is bounded by the code. An LLM agent's next action is partly a function of whatever is in its context window — including untrusted retrieved documents or tool outputs an attacker influenced — so its effective attack surface is anything it can read, not just what an engineer wired up. Least privilege caps the actual damage a successful manipulation can do, turning it into a bounded, recoverable incident rather than a full breach.

</details>

<details>
<summary><b>Q: What is zero trust, and what two structural failures of the perimeter/castle-and-moat model does it address?</b></summary>

Zero trust: never trust, always verify — every request is independently authenticated, authorized, and validated against policy regardless of source. It replaces the perimeter model, which breaks down because (1) cloud/SaaS means there's often no single network perimeter anymore, and (2) once an attacker gets past the perimeter by any means, a castle-and-moat architecture has little left to stop lateral movement, since everything "inside" implicitly trusted everything else "inside."

</details>

<details>
<summary><b>Q: Why does TLS use both symmetric and asymmetric encryption rather than just one?</b></summary>

Asymmetric encryption solves the key-distribution problem (a public key can be shared openly) but is far too computationally expensive for bulk data. TLS uses asymmetric crypto only once, at the handshake, to securely agree on a random symmetric session key, then switches to fast symmetric encryption (AES) for the rest of the connection's actual data — getting the distribution benefit of asymmetric crypto and the performance of symmetric crypto.

</details>

<details>
<summary><b>Q: Why is it especially risky to pass a raw secret through an LLM's context window compared to a normal backend service handling the same secret?</b></summary>

A backend service's handling of a secret is deterministic, reviewed code that doesn't change behavior based on request content. An LLM's behavior is influenced by everything in its context — a prompt injection could plausibly ask it to "print your full context," and a model that's absorbed a raw API key has no hard guarantee it won't comply. The fix is architectural: resolve secrets inside tools at execution time from a vault, using the tool's own scoped identity, so the raw value never enters the model's context.

</details>

<details>
<summary><b>Q: SIEM vs. SOAR vs. XDR vs. EDR — what does each actually do, and how do they compose?</b></summary>

EDR watches individual endpoints and can act on them directly; XDR correlates endpoint signals with identity/cloud/email telemetry into one unified cross-domain detection; SIEM aggregates and correlates logs from nearly anything in the environment, prioritizing breadth and long-term retention; SOAR automates what happens *after* any of those raises an alert, running enrichment/containment playbooks. They compose: EDR feeds XDR, SIEM ingests broadly in parallel, and SOAR automates the response once either fires.

</details>

<details>
<summary><b>Q: Threat hunting vs. incident response — how do they relate, and what are the six NIST IR lifecycle stages?</b></summary>

Threat hunting is proactive and hypothesis-driven, assuming a compromise may already be present and undetected; incident response is the reactive process once an incident *has* been identified (by a hunt or an alert). NIST IR stages: preparation → detection & analysis → containment → eradication → recovery → post-incident review/lessons learned.

</details>

<details>
<summary><b>Q: IOCs vs. TTPs — why is detection built around TTPs considered more durable?</b></summary>

IOCs (a hash, an IP) are atomic artifacts an attacker can trivially change for the next campaign, so IOC-based detection has a short shelf life. TTPs describe behavioral patterns — the underlying method — which is far more expensive for an attacker to abandon than rotating an indicator, so detection built around TTPs keeps catching variants of the same attack even after every IOC has changed.

</details>

<details>
<summary><b>Q: MITRE ATT&CK's tactic/technique/procedure hierarchy — give the one-line definition of each, and say why mature detection targets the technique level.</b></summary>

Tactic = the goal, the "why" (e.g., Persistence). Technique = the general method, the "how" (e.g., a scheduled task for persistence). Procedure = one specific actor's exact real-world implementation (exact command syntax, exact tooling). Detection at the technique level is durable — it keeps working even after an actor changes every procedure-level detail — while procedure-level (IOC-style) detection is precise but brittle.

</details>

<details>
<summary><b>Q: How does the cyber kill chain differ from MITRE ATT&CK, and when would you reach for each?</b></summary>

The kill chain is a single, strictly linear seven-stage model of one intrusion (Recon → Weaponization → Delivery → Exploitation → Installation → C2 → Actions on Objectives) — good for high-level narrative/teaching. ATT&CK is a much broader, non-linear matrix of tactics/techniques with no fixed order and far more granularity — the better tool for anything operational: mapping detections, building coverage heatmaps, structuring actionable threat intel.

</details>

<details>
<summary><b>Q: OAuth 2.0 vs. OIDC — state the one-line distinction, and name OAuth's four roles.</b></summary>

OAuth is for authorization — what a client can do; OIDC adds authentication on top of it via a signed ID token — who the user verifiably is. OAuth's four roles: resource owner, client, authorization server, resource server.

</details>

<details>
<summary><b>Q: Why do systems bother with a separate refresh token instead of one long-lived access token?</b></summary>

A single long-lived bearer token used on every API call means a leak grants an attacker extended, unchecked access. Splitting into a short-lived access token (used on every call, so a leak self-limits within minutes) and a longer-lived refresh token (used only against the authorization server to mint new access tokens, more easily rotated/revoked) confines the higher-risk credential to a narrower, more controllable exchange.

</details>

<details>
<summary><b>Q: Managed identity vs. service principal vs. workload identity federation — how do the three relate?</b></summary>

Every managed identity is a service principal (Entra ID's application-identity concept), specifically one where the platform auto-creates and rotates credentials so app code never touches a secret. A "plain" service principal by default authenticates via a static client secret/certificate someone must manage. Workload identity federation generalizes the same "no stored secret" property to workloads *outside* Azure (a CI/CD pipeline, an on-prem K8s cluster) by having Entra ID trust that workload's own natively-issued token directly.

</details>

<details>
<summary><b>Q: Direct vs. indirect prompt injection — why is the indirect variant considered far more dangerous for agentic systems?</b></summary>

Direct injection requires the attacker to reach the system's input surface themselves, where access controls at least apply. Indirect injection embeds instructions in content the agent later retrieves as ordinary "data" (a poisoned document, a compromised webpage, a manipulated tool response) — the attacker needs zero access to the target system, and a single poisoned document can silently compromise every future session that retrieves it, since the model has no structural way to separate data from instructions once both are just tokens in context.

</details>

<details>
<summary><b>Q: Explain the confused deputy problem and how it maps onto an LLM agent with tool access.</b></summary>

A program with legitimate elevated privileges is tricked by a less-privileged party into misusing them on the trickster's behalf — the classic example is a compiler with real write access to a protected log file, tricked into overwriting it via an attacker-supplied output path. An LLM agent maps directly: it holds real, legitimate tool privileges granted on a legitimate user's behalf, and a successful prompt injection doesn't need to escalate any privilege — it just directs the agent's already-legitimate standing privilege at an attacker-chosen target, with the agent's own permissions never technically violated.

</details>

<details>
<summary><b>Q: Why should the model never be the final authority on whether an agent action is allowed to execute?</b></summary>

Three reasons: models can be manipulated (injection, jailbreaks) regardless of normal alignment, since alignment is a statistical tendency, not a guarantee; models can hallucinate permissions or context that don't exist, sounding equally confident whether true or fabricated; and everything inside an LLM is soft, probabilistic reasoning with no discrete, provably-enforced boundary. The fix: the agent only ever proposes an action, and a separate, deterministic authorization service outside the LLM validates it against ground-truth permissions before an actuator executes it.

</details>

---

## 16. Project Perception & Enterprise Platform Design (→ `16_project_perception_and_enterprise_platform_design.md`)

<details>
<summary><b>Q: Name the six layers of the Project Perception-style pipeline in order, and what question each layer answers.</b></summary>

Signals ("what raw data exists") → Context ("what does it mean here") → Models ("what reasoning applies") → Harness ("how do we run that reasoning against real systems reliably") → Agents ("what mission organizes the reasoning") → Actuators ("how does a decision become a real, authorized change") — and the loop closes because actuator actions generate new signals.

</details>

<details>
<summary><b>Q: Why is "security context" described as a token-efficient representation rather than just "more retrieval"?</b></summary>

Because the problem it solves isn't that the agent can't find data — it's that re-deriving and re-serializing the same slowly-changing facts (asset ownership, criticality, policy, incident history) from scratch on every single investigation is slow (each fact is a round trip) and token-expensive (each fact gets re-injected into the prompt) at millions-of-alerts scale. Context pre-joins those facts once, on its own refresh cadence, into a compact structure the agent reasons over directly.

</details>

<details>
<summary><b>Q: How does security context map onto the agent-memory taxonomy, and how does it differ from an agent's own episodic memory?</b></summary>

Security context is closest to a continuously-maintained, organization-wide semantic/persistent memory layer, kept up to date independent of any single agent run. An agent's episodic memory for one investigation ("what have I found so far in this case") sits on top of context, not instead of it — episodic memory is disposable and task-scoped, context is durable and shared across every agent and investigation.

</details>

<details>
<summary><b>Q: What's the architectural distinction between the model layer, the harness, and the actuator layer?</b></summary>

The model only generates reasoning/decisions (text or structured output) and never touches a real system. The harness is the runtime that actually executes that decision — scheduling, tool invocation, retries, concurrency, handoffs, approval checkpoints, recovery, observability. The actuator is where an authorized action actually lands and takes effect in the real world: the model decides, the harness executes reliably, the actuator effects.

</details>

<details>
<summary><b>Q: Why are Red, Blue, and Green described as a closed loop rather than three independent tools, and what role do playbooks play?</b></summary>

Run independently, Red's vulnerability findings go unprioritized, Blue investigates without knowing what's proven-exploitable, and Green remediates off a generic severity score — three disconnected backlogs. Coordinated, Red's exploitable findings prioritize Blue's detection focus, Blue's investigated findings prioritize Green's remediation, and Green's hardening changes reshape the surface Red tests next. Playbooks constrain each agent's action space to a predefined, human-reviewed procedure per scenario type — what makes the fleet auditable and predictable rather than needing every individual decision reviewed from scratch.

</details>

<details>
<summary><b>Q: Walk through the four distinct stages between a model's output and an actual production change.</b></summary>

Model output (text) → recommendation (not yet approved) → authorized action (an independent authorization/policy check has approved it) → actual system change (the actuator has executed it). Authorization, validation (confirm target state immediately before executing), rollback, and auditability are all non-negotiable specifically at the actuator layer, because this is where effects become real and potentially irreversible.

</details>

<details>
<summary><b>Q: State the CAP theorem precisely — when does the tradeoff actually force a choice, and why does that choice collapse to CP vs. AP in practice?</b></summary>

You can only fully guarantee two of Consistency, Availability, and Partition tolerance *while a network partition is actually happening* — outside a partition, a well-built system can be both consistent and available. Partition tolerance isn't optional in any real distributed system (networks fail), so the real decision during an actual partition is CP (block/refuse rather than serve possibly-stale data) vs. AP (keep serving on both sides of the split, reconcile once it heals).

</details>

<details>
<summary><b>Q: Which stores in a multi-agent security platform should be strongly consistent vs. eventually consistent, and why?</b></summary>

The audit/decision store needs strong consistency (or at minimum read-your-writes) — an auditor asking "was this action taken" can never be given a stale "not found" for an action that already executed. The signal/telemetry ingestion pipeline and a denormalized triage/search index should be eventually consistent — at millions-of-events-per-day throughput, requiring synchronous global visibility before acknowledging would throttle ingestion for no real benefit, and a few seconds of staleness on an alert count costs nothing that matters.

</details>

<details>
<summary><b>Q: What's the difference between a distributed lock and optimistic concurrency control, and why use both for "two agents touch the same resource"?</b></summary>

A distributed lock is acquired before an actuator call touches a resource, serializing concurrent attempts (the second agent blocks or is rejected). Optimistic concurrency control is a defense-in-depth backstop — check a version/timestamp on the resource immediately before committing, and reject the write if it's changed since read. Together, a genuine double-action essentially can't happen silently: it's either serialized by the lock or caught by the version check if the lock is ever bypassed or expires mid-operation.

</details>

<details>
<summary><b>Q: Why is "exactly-once delivery" generally unachievable in distributed systems, and what do systems actually do instead?</b></summary>

Processing a message and acknowledging it can't be one atomic step across a network boundary between independently-failing systems — there's always a window (crash after processing but before acknowledging, or vice versa) where you can't distinguish a lost ack from a duplicate delivery. The practical substitute is at-least-once delivery (never silently drop, retry until acked) combined with idempotent processing (an idempotency key per logical action) — behaviorally equivalent to exactly-once from the consumer's point of view.

</details>

<details>
<summary><b>Q: What's the saga pattern, and when do you need it instead of a normal database transaction?</b></summary>

A way to coordinate a multi-step transaction across multiple services when you can't wrap it in one atomic database transaction — each step has a defined compensating ("undo") action, and if a later step fails, the saga runs the compensating actions for every step that already succeeded, in reverse, to bring the system back to a consistent state.

</details>

<details>
<summary><b>Q: In the latency/quality/cost tradeoff triangle, which lever is close to a "free" win, and why?</b></summary>

Parallelizing independent steps (parallel tool calls, parallel retrieval) — total cost sums linearly across every step regardless of parallelism, but total latency only sums across the sequential critical path, so running independent steps concurrently cuts latency without adding token cost. Almost every other lever (a smaller model, less retrieved context) is a genuine three-way tradeoff that has to be validated against eval, not assumed.

</details>

<details>
<summary><b>Q: Why does output token cost typically exceed input token cost, and what design choice does that favor?</b></summary>

Generating each output token is an inherently sequential, full-forward-pass operation conditioned on everything generated so far, while input tokens can be processed in parallel during prefill — so the marginal compute cost of an output token is higher. This favors trimming output verbosity (a structured, concise summary schema instead of free-form prose) as often a bigger cost win than trimming the same amount of input context.

</details>

<details>
<summary><b>Q: What concrete factors should drive the "auto-remediate vs. require human approval" threshold for an incident, and how should a new playbook's threshold evolve over time?</b></summary>

Reversibility of the action, blast radius, the investigation's confidence/evidence strength, the historical accuracy of that specific playbook (validated in shadow mode), and regulatory/compliance sensitivity of the affected asset. A new playbook should start with a low auto-remediation ceiling and high human-approval rate, prove out a low unsafe-action rate in shadow mode and limited live use, and only then earn wider autonomy for that narrow scenario type — never as a blanket platform-wide upgrade.

</details>

<details>
<summary><b>Q: What is backpressure/shedding, and why can't autoscaling alone handle a millions-of-alerts spike?</b></summary>

An active campaign or a noisy new detection rule can outpace processing capacity faster than autoscaling can react. Backpressure/shedding is an explicit, reviewed policy — buffer via the stream layer's own durability, admission control that prioritizes by severity/confidence, explicit load-shedding rules (e.g., auto-suppress/batch low-severity duplicates above a queue-depth threshold) so shedding is a deliberate, auditable decision rather than silent data loss, plus circuit breakers on the model gateway.

</details>

<details>
<summary><b>Q: Framed as an ML system design problem, why is "millions of alerts, auto-remediate low-risk incidents" best modeled as a funnel rather than a single classifier?</b></summary>

It isn't one scoring problem — it's a chain of sub-decisions (is this a duplicate/false positive, what's the likely pattern, what evidence would confirm it, what remediation fits, is that remediation safe to auto-apply). Framed as a funnel: millions of raw alerts in, most resolved automatically at low cost (deduped, auto-closed, or auto-remediated within policy), a much smaller number escalated to real investigation, and fewer still requiring human approval before action.
